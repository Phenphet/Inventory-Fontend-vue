<script setup>
    ...
    const saveData = () => {
        ...
        console.log(formData.sli)
        console.log(formData.dli)
        console.log(formData.remake) 
        console.log(formData.referenceNumber) 

        if(stockStore.createTransfer(formData)) {
            Swal.fire({
                icon: 'success',
                text: 'เพิ่มข้อมูลเรียบร้อยเเล้ว',
                timer: 2000
            })
            clearForm()
        }else{
    ...
    const btnEdit = (movement_id) => {
        ...
        const data = stockStore.stockDataTranfer.find(items => items.movement_id === movement_id)
        ...
        formData.dli = data.warehouseLocation_DLI.location_id
    ...
    const editData = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                stockStore.updateTransfer(formData)
                Swal.fire({
                    icon: 'success',
    ...
    const btnDelete = (movement_id) => {
         Swal.fire({
            ...
            if (result.isConfirmed) {
                stockStore.deleteTransfer(movement_id)
                Swal.fire({
                    icon: 'success',
...
<template>
    <main>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header row">
                        <h3 class="card-title col-sm-10 pt-2">Table Tranfer All </h3>
                        <div class="col-sm-2">
                            <button class="btn btn-success" data-toggle="modal" data-target="#add" @click="btnAdd()">Add warehouse</button>
                        </div>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <table class="table table-bordered table-hover dataTable dtr-inline " role="grid">
                                        ...
                                        <tbody>
                                            <tr class="" v-for="items in stockStore.stockDataTranfer">
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <th rowspan="1" colspan="1">SLI(ต้นทาง)</th>
                                                <th rowspan="1" colspan="1">DLI(ปลายทาง)</th>
        ...
        <Modal :titleModal="statusModal === 'create' ? 'Create Transfer' : 'Update Transfer'"
            ...
            <div class="form-group">
                <label>Select location (ต้นทาง)</label>
            ...
            <div class="form-group">
                <label>Select location (ปลายทาง)</label>
                <select class="form-control" v-model="formData.dli">
                    <option disabled value="">selected</option>
                    <option v-for="items in warehosueStore.warehouseData" :value="items.location_id">{{ items.name }} |
                        {{ items.address }}</option>
                </select>
